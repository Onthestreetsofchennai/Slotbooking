...
async function loadClientErrorReports() {
  ...
  try {
    await ensureClientErrorTable();
    var rows = await neonSQL(
      'SELECT id,created_at,app_version,source,severity,kind,route,message,stack,user_agent,platform,screen,online,handled FROM client_error_reports ORDER BY handled ASC, created_at DESC LIMIT 80'
    );
...
function mapVenueRows(vRows) {
  return (vRows || []).map(function(v){ return {
    ...
    landmark:v.landmark||v.location||'',
    mapUrl:v.map_url||'',
    imageUrl:v.image_url||v.thumbnail_url||''
  }; });
}
...
const RANKINGS_CACHE_MS = 5 * 60 * 1000;
const VENUES_CACHE_MS = 2 * 60 * 1000;
const PROFILE_CACHE_MS = 5 * 60 * 1000;
const MEMBER_STATUS_POLL_MS = 30000;
const MEMBER_REQUESTS_POLL_MS = 30000;
const GIG_SCHEDULE_REFRESH_MS = 60000;
const PHOTO_LIGHT_SYNC_MS = 2 * 60 * 1000;
const LIVE_FAST_MS = ADMIN_DASHBOARD_REFRESH_MS;
const LIVE_STATUS_MS = MEMBER_STATUS_POLL_MS;
const LIVE_GIG_MS = GIG_SCHEDULE_REFRESH_MS;
...
function liveCoreBookingsQuery() {
  ...
  if (otsIsAdminApp()) {
    return {
      sql: 'SELECT ' + cols + ' FROM bookings ORDER BY created_at DESC LIMIT 300',
      params: []
    };
  }
  var myPhone = _normPhone(memberPhone || '');
...
async function refreshLiveCoreDataNow(opts) {
  ...
  try {
    vRows = await neonSQL(
      'SELECT id,name,day,date,time_start,time_end,status,venue_type,landmark AS location,landmark,map_url,image_url AS thumbnail_url,image_url FROM venues ORDER BY date ASC, time_start ASC LIMIT 700'
    );
  } catch (e) {
    if (!opts._retry) {
...
function showPage(page) {
  ...
  if (page === 'myrequests') {
    // Render from cache immediately, then fetch live; keep a slow poll only while this page is open.
    renderUserBookings();
    fetchMyBookingsLive();
    startMyRequestsPoll();
  ...
  if (page === 'admin') {
    ...
    _adminNotifTimer = setInterval(function(){ if (!document.hidden) updateAdminNotifCount(); }, NOTIFICATION_POLL_MS);
  } else {
    clearInterval(_adminNotifTimer);
...
function startAdminPolling() {
  ...
  // Immediate first refresh so admin sees live data right away.
  if (adminLoggedIn) refreshAdmin();
  _adminPollTimer = setInterval(function() {
    if (!adminLoggedIn) return;
    // Notification poll only: pending queue metadata, no full dashboard/table reload.
    neonSQL(
      ...
      .then(function(rows) {
        var pendingBookings = rows.filter(function(r){ return r.type === 'booking'; }).map(function(r){ return {
          id: r.ref_id, venue: r.venue || '', date: r.date || '', name: r.name || '', status: 'pending', createdAt: r.created_at || ''
        }; });
        checkNewBookingsAndNotify(pendingBookings);
        var countEl = document.getElementById('adminNotifCount');
        if (countEl) {
          if (rows.length) { countEl.style.display=''; countEl.textContent = rows.length > 99 ? '99+' : rows.length; }
          else { countEl.style.display = 'none'; }
        }
        if (_adminNotifOpen) loadAdminNotifPanel();
      })
      .catch(function(){});
  }, NOTIFICATION_POLL_MS);
  _adminDashboardTimer = setInterval(function() {
    if (!adminLoggedIn || document.hidden || !currentAdminTab) return;
    if (['photos','members','reports','superadmin'].indexOf(currentAdminTab) > -1) return;
    refreshAdmin();
  }, ADMIN_DASHBOARD_REFRESH_MS);
}
function stopAdminPolling() {
  if (_adminPollTimer) { clearInterval(_adminPollTimer); _adminPollTimer = null; }
  if (_adminDashboardTimer) { clearInterval(_adminDashboardTimer); _adminDashboardTimer = null; }
  _knownBookingIds = null;
}

let _liveKeepWarmTimer = null;
function startLiveKeepWarm() {
  // Disabled on purpose: keep-warm pings prevent Neon scale-to-zero and add CU-hours.
  _liveKeepWarmTimer = null;
}
...
function refreshLiveOnFocus() {
  ...
  var now = Date.now();
  if (now - _lastFocusLiveRefreshAt < 60000) return;
  _lastFocusLiveRefreshAt = now;
  if (adminLoggedIn) {
    updateAdminNotifCount();
    if (currentAdminTab && ['photos','members','reports','superadmin'].indexOf(currentAdminTab) === -1) refreshAdmin();
    return;
  }
  if (memberLoggedIn) {
    if (currentPageName() === 'myrequests') fetchMyBookingsLive();
    var page = currentPageName();
    if (page === 'venues' || page === 'home') {
      refreshLiveCoreData({ showLoading:false, silentIfCached:true, maxAgeMs:VENUES_CACHE_MS }).catch(function(){});
    }
  } else {
    refreshLiveCoreData({ showLoading:false, silentIfCached:true, maxAgeMs:VENUES_CACHE_MS }).catch(function(){});
  }
...
async function loadLeaderboard() {
  ...
  try {
    var cachedPayload = JSON.parse(localStorage.getItem(cacheKey) || 'null');
    var cached = Array.isArray(cachedPayload) ? cachedPayload : (cachedPayload && Array.isArray(cachedPayload.rows) ? cachedPayload.rows : []);
    var cachedZones = cachedPayload && Array.isArray(cachedPayload.zoneRows) ? cachedPayload.zoneRows : [];
    var cachedAt = cachedPayload && cachedPayload.ts ? Number(cachedPayload.ts) : 0;
    cacheFresh = !!(cached.length && cachedAt && (Date.now() - cachedAt) < RANKINGS_CACHE_MS);
    if (cached.length) {
      _lbAllRows = cached;
      _lbZoneRows = cachedZones;
  ...
  try {
    ...
    try { localStorage.setItem(cacheKey, JSON.stringify({ ts:Date.now(), rows:_lbAllRows, zoneRows:_lbZoneRows })); } catch(_){}
...
async function runMigrations(force) {
  ...
  await run("CREATE INDEX IF NOT EXISTS idx_members_email ON members(LOWER(email))");
  await run("CREATE INDEX IF NOT EXISTS idx_members_phone ON members(phone)");
  await run("CREATE INDEX IF NOT EXISTS idx_bookings_phone ON bookings(phone)");
  await run("CREATE INDEX IF NOT EXISTS idx_bookings_email ON bookings(LOWER(email))");
  await run("CREATE INDEX IF NOT EXISTS idx_slot_requests_user_id ON bookings(phone)");
  await run("CREATE INDEX IF NOT EXISTS idx_bookings_date_status ON bookings(date, status)");
  await run("CREATE INDEX IF NOT EXISTS idx_bookings_venue_id ON bookings(venue_id)");
  await run("CREATE INDEX IF NOT EXISTS idx_notifications_user_created ON dismissed_notifications(user_key, created_at DESC)");
  await run("CREATE INDEX IF NOT EXISTS idx_venues_date ON venues(date)");
  await run("CREATE INDEX IF NOT EXISTS idx_claims_member_created ON claims(member_phone, created_at DESC)");
  await run("CREATE INDEX IF NOT EXISTS idx_claims_status_created ON claims(status, created_at DESC)");
  await run("CREATE INDEX IF NOT EXISTS idx_rankings_zone ON claims(zone_name)");
...
async function handleCommunityAdImageUpload(e) {
  ...
  showToast('', 'Use Image URL', 'To reduce Neon cost, community news images now save only HTTPS image URLs.');
  addCommunityAdImageUrlPrompt();
  return;
...
function addCommunityAdImageUrlPrompt() {
  if (!requireAdminPerm('ads', 'community news')) return false;
  var url = window.prompt('Paste the Cloudinary/HTTPS image URL');
  url = String(url || '').trim();
  if (!url) return false;
  if (typeof isRemoteMediaUrl === 'function' && (!isRemoteMediaUrl(url) || isBase64MediaUrl(url))) {
    showToast('', 'Use Image URL', 'Please paste a public HTTPS image URL, not a Base64 image.');
    return false;
  }
  _communityAdImageUrl = url;
  renderCommunityAdImagePreview();
  var status = document.getElementById('communityAdStatus');
  if (status) status.textContent = 'Image URL added. Publish to save it.';
  return false;
}
...
function monthlyReportSafeFilename(text, ext) {
  var base = String(text || 'OTS Monthly Report')
    .replace(/[^\w\s-]+/g, '')
    .replace(/\s+/g, '_')
    .replace(/_+/g, '_')
    .replace(/^_+|_+$/g, '')
    .slice(0, 90) || 'OTS_Monthly_Report';
  return base + '.' + (ext || 'pdf');
}

function monthlyReportDownloadBlob(blob, filename) {
...
function monthlyReportPdfAscii(text) {
...
function monthlyReportPdfEscape(text) {
  return monthlyReportPdfAscii(text).replace(/\\/g, '\\\\').replace(/\(/g, '\\(').replace(/\)/g, '\\)');
}

function monthlyReportPdfBytesFromString(str) {
  var bytes = new Uint8Array(str.length);
  for (var i = 0; i < str.length; i++) bytes[i] = str.charCodeAt(i) & 255;
  return bytes;
}

function monthlyReportPdfConcat(parts) {
...
function monthlyReportPdfStream(dict, bytes) {
  return monthlyReportPdfConcat([
    monthlyReportPdfBytesFromString(dict.replace(/\/Length\s+0/, '/Length ' + bytes.length) + '\nstream\n'),
    bytes,
    monthlyReportPdfBytesFromString('\nendstream')
  ]);
}

function monthlyReportPdfDataUrlBytes(dataUrl) {
...
async function monthlyReportPdfImageAsset(url, maxSide, quality) {
  if (!url) return null;
  try {
    var blob = await monthlyReportPhotoBlob(url);
    if (!blob) return null;
    var bitmap = typeof createImageBitmap === 'function' ? await createImageBitmap(blob) : null;
    var sourceWidth = bitmap ? bitmap.width : 0;
    var sourceHeight = bitmap ? bitmap.height : 0;
    if (!bitmap) {
    ...
    var scale = Math.min(1, (maxSide || 1500) / Math.max(sourceWidth || 1, sourceHeight || 1));
    var canvas = document.createElement('canvas');
    canvas.width = Math.max(1, Math.round((sourceWidth || 1) * scale));
    canvas.height = Math.max(1, Math.round((sourceHeight || 1) * scale));
    ...
    var dataUrl = canvas.toDataURL('image/jpeg', quality || 0.86);
    return {
      bytes: monthlyReportPdfDataUrlBytes(dataUrl),
      width: canvas.width,
      height: canvas.height
    };
  } catch(e) {
    console.warn('[OTS] report PDF image skipped:', e && (e.message || e));
    return null;
  }
}

function monthlyReportPdfFitRect(img, boxW, boxH) {
  if (!img || !img.width || !img.height) return { width:0, height:0 };
  var scale = Math.min(boxW / img.width, boxH / img.height);
  return { width: img.width * scale, height: img.height * scale };
}

function monthlyReportPdfText(x, y, size, text, options) {
  options = options || {};
  var font = options.bold ? '/F2' : '/F1';
  var align = options.align || 'left';
  var safe = monthlyReportPdfAscii(text);
  var drawX = x;
  if (align !== 'left') {
    var approxWidth = safe.length * size * 0.52;
    drawX = align === 'center' ? x - (approxWidth / 2) : x - approxWidth;
  }
  return 'BT ' + font + ' ' + size + ' Tf 1 0 0 1 ' + drawX.toFixed(2) + ' ' + y.toFixed(2) + ' Tm (' + monthlyReportPdfEscape(safe) + ') Tj ET\n';
}

function monthlyReportPdfWrap(text, maxChars) {
  var words = monthlyReportPdfAscii(text).split(/\s+/).filter(Boolean);
  var lines = [];
  var line = '';
...
function monthlyReportPdfImage(name, x, y, w, h) {
  return 'q ' + w.toFixed(2) + ' 0 0 ' + h.toFixed(2) + ' ' + x.toFixed(2) + ' ' + y.toFixed(2) + ' cm /' + name + ' Do Q\n';
}

function monthlyReportPdfRect(x, y, w, h, gray) {
  gray = gray == null ? 0.92 : gray;
  return gray + ' g ' + x.toFixed(2) + ' ' + y.toFixed(2) + ' ' + w.toFixed(2) + ' ' + h.toFixed(2) + ' re f\n0 g\n';
}

async function buildMonthlyReportPdfBlob(ctx, rows) {
  var pageW = 841.89;
  var pageH = 595.28;
  var margin = 38;
  var summary = computeMonthlyReportSummary(rows);
  var generated = new Date().toLocaleString('en-IN', { dateStyle:'medium', timeStyle:'short' });
  var logoSources = [
    new URL('ots-brand-mark.png', location.href).href,
    new URL('gcc-logo.png', location.href).href,
    new URL('chennai-smart-city.png', location.href).href
  ];
  var assets = [];
  for (var l = 0; l < logoSources.length; l++) {
    assets.push({ key:'Logo' + (l + 1), image: await monthlyReportPdfImageAsset(logoSources[l], 600, 0.88) });
  }
  for (var r = 0; r < rows.length; r++) {
    assets.push({ key:'Photo' + (r + 1), image: rows[r].photoUrl ? await monthlyReportPdfImageAsset(rows[r].photoUrl, 1800, 0.86) : null });
  }
  ...
  var catalogId = addObject(null);
  var pagesId = addObject(null);
  var fontId = addObject(monthlyReportPdfBytesFromString('<< /Type /Font /Subtype /Type1 /BaseFont /Helvetica >>'));
  var boldFontId = addObject(monthlyReportPdfBytesFromString('<< /Type /Font /Subtype /Type1 /BaseFont /Helvetica-Bold >>'));
  var imageRefs = {};
  assets.forEach(function(asset) {
    if (!asset.image) return;
    var dict = '<< /Type /XObject /Subtype /Image /Width ' + asset.image.width + ' /Height ' + asset.image.height + ' /ColorSpace /DeviceRGB /BitsPerComponent 8 /Filter /DCTDecode /Length 0 >>';
    var id = addObject(monthlyReportPdfStream(dict, asset.image.bytes));
    imageRefs[asset.key] = { id:id, image:asset.image };
  });
  var xObjects = Object.keys(imageRefs).map(function(key){ return '/' + key + ' ' + imageRefs[key].id + ' 0 R'; }).join(' ');
  var pageIds = [];

  function logosContent() {
    var c = '';
    var ots = imageRefs.Logo1;
    var gcc = imageRefs.Logo2;
    var smart = imageRefs.Logo3;
    if (gcc) {
      var g = monthlyReportPdfFitRect(gcc.image, 54, 54);
      c += monthlyReportPdfImage('Logo2', pageW - 74, pageH - 72, g.width, g.height);
    }
    if (smart) {
      var s = monthlyReportPdfFitRect(smart.image, 132, 46);
      c += monthlyReportPdfImage('Logo3', pageW - 226, pageH - 68, s.width, s.height);
    }
    if (ots) {
      var o = monthlyReportPdfFitRect(ots.image, 50, 50);
      c += monthlyReportPdfImage('Logo1', pageW - 292, pageH - 70, o.width, o.height);
    }
    return c;
  }

  function addPage(content) {
    var contentId = addObject(monthlyReportPdfStream('<< /Length 0 >>', monthlyReportPdfBytesFromString(content)));
    var resources = '<< /Font << /F1 ' + fontId + ' 0 R /F2 ' + boldFontId + ' 0 R >>' + (xObjects ? ' /XObject << ' + xObjects + ' >>' : '') + ' >>';
    var page = '<< /Type /Page /Parent ' + pagesId + ' 0 R /MediaBox [0 0 ' + pageW + ' ' + pageH + '] /Resources ' + resources + ' /Contents ' + contentId + ' 0 R >>';
    var pageId = addObject(monthlyReportPdfBytesFromString(page));
    pageIds.push(pageId);
  }

  var cover = logosContent();
  cover += monthlyReportPdfText(pageW / 2, 330, 34, ctx.title, { bold:true, align:'center' });
  cover += monthlyReportPdfText(pageW / 2, 292, 15, ctx.typeLabel + ' / Generated ' + generated, { align:'center' });
  cover += monthlyReportPdfText(pageW / 2, 258, 12, 'On The Streets of Chennai', { align:'center' });
  addPage(cover);

  var sum = logosContent();
  sum += monthlyReportPdfText(margin, 482, 28, 'Summary', { bold:true });
  var summaryRows = [
    ['Number of Venues', summary.venues],
    ['Number of Shows', summary.shows],
    ['Number of Bands', summary.bands],
    ['Number of Days Performed', summary.days],
    ['Total Footfall', summary.totalFootfall],
    ['Average Footfall per Show', summary.averageFootfall]
  ];
  var y = 430;
  summaryRows.forEach(function(row) {
    sum += monthlyReportPdfRect(margin, y - 12, 520, 31, 0.96);
    sum += monthlyReportPdfText(margin + 16, y, 14, row[0], { bold:true });
    sum += monthlyReportPdfText(margin + 500, y, 14, String(row[1]), { bold:true, align:'right' });
    y -= 39;
  });
  addPage(sum);

  var divider = logosContent();
  divider += monthlyReportPdfRect(0, 0, pageW, pageH, 0.96);
  divider += logosContent();
  divider += monthlyReportPdfText(pageW / 2, 318, 30, ctx.typeLabel, { bold:true, align:'center' });
  divider += monthlyReportPdfText(pageW / 2, 278, 24, 'Monthly Report - ' + monthLabelFromKey(ctx.monthKey, false), { bold:true, align:'center' });
  addPage(divider);
  ...
  rows.forEach(function(row, idx) {
    ...
    var week = monthlyReportWeekLabel(row.date);
    if (week !== prevWeek) {
      c += monthlyReportPdfText(margin, topY, 20, week, { bold:true });
      topY -= 34;
      prevWeek = week;
    }
    var leftX = margin;
    ...
    var lineY = topY;
    var details = [
      ['' + (idx + 1) + '. Date :', row.reportDateLabel || monthlyReportDefaultDateLabel(row)],
      ['Band name :', row.teamName],
      ['Foot fall :', String(row.footfall || 0)]
    ];
    details.forEach(function(item) {
      c += monthlyReportPdfText(leftX, lineY, 15, item[0], { bold:true });
      c += monthlyReportPdfText(leftX + (item[0].length > 10 ? 118 : 92), lineY, 15, item[1]);
      lineY -= 28;
    });
    lineY = topY;
    [
      ...
      c += monthlyReportPdfText(rightX, lineY, 15, item[0], { bold:true });
      monthlyReportPdfWrap(item[1], 38).slice(0, 2).forEach(function(line, i) {
        c += monthlyReportPdfText(rightX + 88, lineY - (i * 20), 15, line);
      });
      lineY -= 32;
    });
    var boxX = margin;
    ...
    var boxH = 318;
    c += monthlyReportPdfRect(boxX, boxY, boxW, boxH, 0.985);
    var photoRef = imageRefs['Photo' + (idx + 1)];
    if (photoRef) {
      var fit = monthlyReportPdfFitRect(photoRef.image, boxW - 26, boxH - 26);
      c += monthlyReportPdfImage('Photo' + (idx + 1), boxX + (boxW - fit.width) / 2, boxY + (boxH - fit.height) / 2, fit.width, fit.height);
    } else {
      c += monthlyReportPdfText(pageW / 2, boxY + (boxH / 2), 18, 'Photo not attached', { align:'center' });
    }
    addPage(c);
  });

  objects[catalogId - 1] = monthlyReportPdfBytesFromString('<< /Type /Catalog /Pages ' + pagesId + ' 0 R >>');
  objects[pagesId - 1] = monthlyReportPdfBytesFromString('<< /Type /Pages /Kids [' + pageIds.map(function(id){ return id + ' 0 R'; }).join(' ') + '] /Count ' + pageIds.length + ' >>');

  var parts = [monthlyReportPdfBytesFromString('%PDF-1.4\n')];
  var offsets = [0];
  objects.forEach(function(obj, index) {
    offsets[index + 1] = parts.reduce(function(sum, part){ return sum + part.length; }, 0);
    parts.push(monthlyReportPdfBytesFromString((index + 1) + ' 0 obj\n'));
    parts.push(obj);
    parts.push(monthlyReportPdfBytesFromString('\nendobj\n'));
  });
  ...
  parts.push(monthlyReportPdfBytesFromString(xref));
  return new Blob([monthlyReportPdfConcat(parts)], { type:'application/pdf' });
}

async function downloadMonthlyReportDirectPDF() {
  if (!requireAdminPerm('reports', 'monthly report download')) return;
  initMonthlyReportControls();
  var ctx = getMonthlyReportContext();
  ...
  if (rows.length) renderMonthlyReportRows(ctx, rows);
  if (!rows.length) {
    showToast('', 'No Report Data', 'No confirmed shows found for this month and type.');
    return;
  }
  showToast('', 'Preparing Download', 'Building the report PDF. Please wait...');
  try {
    await hydrateMonthlyReportPhotos(rows);
    await verifyMonthlyReportPhotos(rows);
    await normalizeMonthlyReportPhotos(rows);
    var blob = await buildMonthlyReportPdfBlob(ctx, rows);
    monthlyReportDownloadBlob(blob, monthlyReportSafeFilename(ctx.title || 'OTS Monthly Report', 'pdf'));
    showToast('', 'Report Downloaded', 'The PDF download has started.');
    logAdminAction('monthly_report_download', ctx.title + ' / ' + rows.length + ' shows').catch(function(){});
  } catch(e) {
    console.error('[OTS] monthly report direct download:', e);
    showToast('', 'Download Failed', 'Could not download the PDF. Please try View Report once.');
  }
}
...
async function downloadMonthlyReportPDF() {
  return downloadMonthlyReportDirectPDF();
}
...
// -- Neon DB: front-page photos store HTTPS media URLs only. Base64 is too large for production-cost use. --

function markPerfPhotosLocalEdit() {
  _perfPhotoLocalEditAt = Date.now();
}

function isBase64MediaUrl(url) {
  return /^data:(image|video)\//i.test(String(url || '').trim());
}
...
function isLegacyImageDataUrl(url) {
  return /^data:image\//i.test(String(url || '').trim());
}

function isDisplayablePerfPhotoUrl(url) {
  return isRemoteMediaUrl(url) || isLegacyImageDataUrl(url);
}

function filterRemotePerfPhotos(list) {
  return (list || []).filter(function(p) {
    return p && p.id && p.dataUrl && !isBase64MediaUrl(p.dataUrl) && isRemoteMediaUrl(p.dataUrl);
  });
}

function filterDisplayablePerfPhotos(list) {
  return (list || []).filter(function(p) {
    return p && p.id && p.dataUrl && isDisplayablePerfPhotoUrl(p.dataUrl);
  });
}
...
function cachePerfPhotoPreview() {
  try {
    localStorage.setItem('ots_perf_photos', JSON.stringify(filterRemotePerfPhotos(perfPhotos).slice(0, PERF_HOME_LIMIT)));
  } catch(e) {}
}

function cachePerfPhotosFull() {
  otsIdbSet('perf_photos_full', filterRemotePerfPhotos(perfPhotos)).catch(function(){});
}
...
async function loadPerfPhotos() {
  ...
  try {
    ...
    if (cachedFull && cachedFull.length) {
      perfPhotos = filterRemotePerfPhotos(_filterDeletedPerfPhotos(cachedFull));
      renderPerfStrip();   // visible immediately
      fastRenderPhotoManager();
    } else {
      const cached = JSON.parse(localStorage.getItem('ots_perf_photos') || '[]');
      if (cached.length) {
        perfPhotos = filterRemotePerfPhotos(cached);
        renderPerfStrip();
        fastRenderPhotoManager();
      }
    }
  ...
  try {
    const perf = await neonSQL(
      "SELECT id,url,caption FROM gallery WHERE id LIKE 'perf_%' ORDER BY id DESC LIMIT 60"
    );
    if (perf.length) {
      var remotePhotos = perf.map(r => ({ id: r.id, dataUrl: r.url, label: r.caption || '' }));
      perfPhotos = filterDisplayablePerfPhotos(mergePerfPhotoLists(perfPhotos, remotePhotos));
...
async function savePerfPhotos() {
  ...
  perfPhotos = filterDisplayablePerfPhotos(perfPhotos);
  ...
  try {
    ...
    const rows = filterRemotePerfPhotos(perfPhotos).map(p => ({ id: p.id, url: p.dataUrl, caption: p.label || '' }));
    if (rows.length) {
...
async function saveSinglePerfPhoto(photo) {
  ...
  if (isBase64MediaUrl(photo.dataUrl) || !isRemoteMediaUrl(photo.dataUrl)) {
    throw new Error('Front-page photos must be saved as HTTPS image URLs, not Base64 data.');
  }
...
async function addPerfPhotoUrlPrompt() {
  if (!requireAdminPerm('photos', 'photo management')) return false;
  var url = window.prompt('Paste the Cloudinary/HTTPS image URL');
  url = String(url || '').trim();
  if (!url) return false;
  if (!isRemoteMediaUrl(url) || isBase64MediaUrl(url)) {
    showToast('', 'Use Image URL', 'Please paste a public HTTPS image URL, not a file or Base64 image.');
    return false;
  }
  var label = String(window.prompt('Photo caption/team name', '') || '').trim() || 'Photos';
  var photo = { id:'perf_' + Date.now() + '_' + Math.random().toString(36).slice(2, 6), dataUrl:url, label:label };
  await saveSinglePerfPhoto(photo);
  perfPhotos.unshift(photo);
  perfPhotos = filterDisplayablePerfPhotos(perfPhotos);
  cachePerfPhotoPreview();
  cachePerfPhotosFull();
  fastRenderPhotoManager();
  renderPerfStrip();
  showToast('', 'Photo Added', 'Front-page photo URL saved.');
  logAdminAction('upload_photo_url', label).catch(function(){});
  return false;
}
...
async function handlePhotoUpload(e) {
  ...
  e.target.value = '';
  showToast('', 'Use Cloudinary URL', 'To reduce Neon cost, front-page photos now save only HTTPS image URLs. Use Add Photo URL.');
  return;
...
async function mlCheckMemberByEmail(email) {
  try {
    ...
    var rows = await neonSQL(
      'SELECT id,name,email,phone,address,bio,avatar_url,role_type,instrument,instagram,blood_group,date_of_birth,zone_current,zone_request,zone_request_reason,zone_request_status,id_proof_url,active FROM members WHERE LOWER(email) = $1 AND active IS NOT FALSE LIMIT 1',
      [lc]
    );
...
async function loadMembers() {
  try {
    ...
    const rows = _dedupeMemberRows(await neonSQL(
      'SELECT id,name,email,phone,address,bio,role_type,instrument,instagram,blood_group,date_of_birth,zone_current,zone_request,zone_request_reason,zone_request_status,id_proof_url,created_at,active FROM members ORDER BY created_at DESC LIMIT 1200'
    ));
    members = rows.map(m => ({
      id:      m.id,
      name:    m.name  || '',
